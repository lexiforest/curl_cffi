...
2. [`benchmark`](benchmark/ws_bench_2.py) - Generates a large file full of random bytes and its hash. When ran as a server, it loads in the file and hash. The client connects and either downloads or uploads this file. The receiving end calculates the file hash to make sure it matches the disk file.

> These do not factor in real internet conditions and run on loopback interfaces with really high MTU sizes

To run them you need Python 3.11+, start by generating a self-signed cert:
...
By default, both benchmarks are configured to download `10GB` with a `65535` byte chunk size. You can edit the code to change these values as well as enabling upload. You can also enable concurrent upload/download, just uncomment the relevant code.
...
> Ensure you have enough disk space and RAM as the entire file is loaded in RAM
